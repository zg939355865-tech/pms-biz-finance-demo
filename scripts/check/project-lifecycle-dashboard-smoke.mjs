...
try {
  for (const width of [1440, 2048]) {
    ...
    const contracts = [
      ['requirement','需求编号,需求名称,需求提出人,当前责任人,状态',186],
      ['proposal','方案编号,方案名称,方案编写人,当前责任人,状态',84],
      ['notice','通知单号,通知名称,销售负责人,当前责任人,状态',62],
      ['project','项目编码,项目名称,项目经理,当前责任人,状态',48],
    ...
    for (const [id, headers, count, total] of contracts) {
      ...
      const dialogLayout = await page.locator('#cockpit-dialog').evaluate(el=>({width:el.getBoundingClientRect().width, rowHeight:el.querySelector('tbody tr').getBoundingClientRect().height, columns:el.querySelectorAll('col').length}));
      expect(Math.abs(dialogLayout.width - ({2:560,3:680,4:960,5:1040}[headers.split(',').length])) < 2, `${id}: 弹窗宽度未按字段数适配`);
      expect(Math.abs(dialogLayout.rowHeight - 56) < 2 && dialogLayout.columns === headers.split(',').length, `${id}: 行高或列宽配置不正确`);
      if (id === 'proposal' || id === 'industry-trend' || id === 'demand-trend') await page.screenshot({path:path.join(reportDir,`dialog-${id}-${width}.png`)});
      if (total === undefined) {
      ...
      if (total !== undefined) {
        let sum = 0, collected = [], collectedMonths = [];
        while (true) {
          ...
          collectedMonths.push(...monthCells);
        ...
        expect(collectedMonths.every((m,i)=>i===0||collectedMonths[i-1]>=m), `${id}: 月份未跨页倒序排列`);
      }
    ...
    if (width === 1440) {
      ...
      expect((await page.locator('#cockpit-dialog th').allTextContents()).join(',') === '方案编号,方案名称,方案编写人,当前责任人,状态', '状态弹窗未沿用业务字段');
      await page.keyboard.press('Escape');
      ...
      expect(await page.locator('#cockpit-dialog td').getAttribute('colspan')==='5', '业务空表列数错误');
      await page.keyboard.press('Escape');
