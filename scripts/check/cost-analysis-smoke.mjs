...
try {
  for (const testCase of activeCases) {
    for (const width of [1440, 2048]) {
      ...
      await page.locator('.page-loading:not(.hidden)').waitFor({state:'hidden'});

      const table = page.locator(testCase.table);
      const search = page.locator(testCase.search);
      expect(await table.locator('button.btn[data-act="export"] i.fa-solid.fa-file-export').count() === 1, '导出按钮未使用公共按钮和标准导出图标');
      const visibleRows = table.locator('tbody > tr:visible:not([data-filter-empty-row])');
      expect(await visibleRows.count() === 5, '初次进入应展示列表前5条数据');
      expect((await table.locator('[data-record-count]').innerText()).trim() === '共 24 条记录', '初次列表应展示预选两个对象的全年24条记录');
      expect(await page.locator('[data-series-key]').count() === 2, '初次进入应展示两条演示曲线');
      expect(await page.locator('[data-series-key]').first().locator('circle').count() === 12, '初次趋势应展示全年12个月');
      expect(await page.locator('.pro-chart-legend-item').count() === 2, '初次趋势应显示两个图例');
      await page.screenshot({path:path.join(reportDir, testCase.code + '-cost-initial-' + width + '.png'),fullPage:true});
      ...
      const totalsMatch = await table.evaluate(root => {
        const rows = [...root.querySelectorAll('tbody [data-row-value][data-filter-match="true"]')].map(row => JSON.parse(row.dataset.rowValue));
        return [...root.querySelectorAll('[data-footer-summary-column]')].every(cell => Math.abs(Number(cell.textContent.replaceAll(',', '')) - rows.reduce((sum,row)=>sum+Number(row[cell.dataset.footerSummaryColumn]||0),0)) < .01);
      });
      expect(totalsMatch, '合计行与全部查询结果不一致');
      ...
      if (testCase.code === 'project') {
        ...
        const currentAccountingPeriod = '2026-09';
        ...
        expect(await page.locator('#project-cost-trend-chart .pro-chart-legend').count() === 1, '趋势图仍显示图例');
        expect(await search.locator('[data-field="costCategory"] option:checked').innerText() === '成本总计', '成本类型默认文案错误');
        const exportAlignment = await table.evaluate(root => {
          const title = root.querySelector('.section-title').getBoundingClientRect();
          const button = root.querySelector('[data-act="export"]').getBoundingClientRect();
          return Math.abs(title.y + title.height / 2 - button.y - button.height / 2) < 2;
        });
        expect(exportAlignment, '导出按钮未与标题同行');
      ...
      if (testCase.code === 'department') {
        ...
        expect(await page.locator('#department-cost-trend-chart .pro-chart-legend').count() === 1, '部门趋势图不应显示图例');
        expect(await search.locator('[data-field="costCategory"] option:checked').innerText() === '成本总计', '成本类型默认文案错误');
        const exportAligned = await table.evaluate(root => {
          const title = root.querySelector('.section-title').getBoundingClientRect();
          const button = root.querySelector('[data-act="export"]').getBoundingClientRect();
          return Math.abs((title.top + title.bottom) / 2 - (button.top + button.bottom) / 2) < 4 && button.left > title.right;
        });
        expect(exportAligned, '部门导出按钮未与标题同行靠右');
        const frozen = await table.evaluate(root => {
          const scroll = root.querySelector('.pro-table-scroll');
        ...
        expect(Math.abs(trendAndSearchWidths.search - trendAndSearchWidths.trend) <= 1, `${testCase.code}@${width}: 趋势图未独占整行`);
        expect(trendAndSearchWidths.plot >= trendAndSearchWidths.trend * 0.6, `${testCase.code}@${width}: 趋势图绘图区宽度不足`);
      ...
      if (width === 1440) {
        if (testCase.code === 'project') {
          ...
          await search.locator('[data-act="query"]').click();
          expect((await table.locator('[data-record-count]').innerText()).trim() === '共 0 条记录', `${testCase.code}: 无查询条件时不应加载列表`);
          await search.locator('[data-act="toggle-multi-select"]').click();
          await search.locator('[data-multi-option][value="PRJ2026002"]').check();
          await search.locator('[data-act="query"]').click();
          expect((await table.locator('[data-record-count]').innerText()).trim() === '共 12 条记录', `${testCase.code}: 项目下拉筛选未生效`);
          ...
          expect((await table.locator('[data-record-count]').innerText()).trim() === '共 24 条记录', '多项目列表筛选失败');
          ...
          expect(await page.locator('[data-series-key]').count() === 0, '重置未清空项目曲线');
        } else {
          ...
          expect(await page.locator('#department-cost-trend-chart .pro-chart-legend').count() === 1, '查询后不应恢复图例');
          ...
          expect((await table.locator('[data-record-count]').innerText()).trim() === '共 0 条记录', `${testCase.code}: 无查询条件时不应加载列表`);
          await page.waitForTimeout(80);
          expect(await page.locator('[data-series-key]').count() === 0, '空条件不应展示曲线');
          ...
          expect(await page.locator('[data-series-key]').first().locator('circle').count() === 12, '清空期间未展示完整月份趋势');
          await search.locator('[data-field="accountingPeriodRangeStart"]').fill('2030-01');
          await search.locator('[data-field="accountingPeriodRangeEnd"]').fill('2030-02');
          await search.locator('[data-act="query"]').click();
          await page.waitForTimeout(80);
          ...
          expect(await page.locator('[data-series-key]').count() === 0, '重置未清空部门曲线');
          expect(await search.locator('[data-field="costCategory"] option:checked').innerText() === '成本总计', '重置后成本类型文案错误');
          expect(await page.locator('#department-cost-trend-chart .pro-chart-legend').count() === 0, '重置后图例应为空');
          await search.locator('[data-field="accountingPeriodRangeStart"]').fill('2026-09');
          await search.locator('[data-field="accountingPeriodRangeEnd"]').fill('2026-09');
          await search.locator('[data-act="query"]').click();
