...
export function renderProjectCockpit(page) {
  ...
  const panel = (id, title, subtitle, extra = '') => `<section class="cockpit-panel" id="${id}" data-component="ProChartCard"><header class="panel-heading"><div><h2>${title}</h2></div>${page.cockpit.dialogs[id] ? `<button class="chart-data" data-view="${id}" aria-label="查看${title}数据">数据 ↗</button>` : ''}</header>${extra}<div class="chart-content"></div></section>`;
  ...
  <dialog id="cockpit-dialog" aria-labelledby="cockpit-dialog-title"><div class="dialog-heading"><h2 id="cockpit-dialog-title"></h2><button id="cockpit-dialog-close" aria-label="关闭">×</button></div><div id="cockpit-dialog-content"></div><footer id="cockpit-dialog-footer"></footer></dialog>
