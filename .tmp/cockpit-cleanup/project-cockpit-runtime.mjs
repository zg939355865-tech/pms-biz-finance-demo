export function projectCockpitRuntime(config) {
  ...
  const monthControl = $('#cockpit-month');
  ...
  $('#cockpit-definitions').onclick = () => {
    $('#cockpit-dialog-title').textContent = '统计口径';
    $('#cockpit-dialog-content').innerHTML = config.definitions.map(x => `<p>${escape(x)}</p>`).join('');
    $('#cockpit-dialog-footer').innerHTML = '';
    dialog.showModal();
  };
  ...
  function render() {
    const selected = monthControl.value || baseMonth;
    ...
    $('#cockpit-range').textContent = `${months[0].replace('-','.')} — ${months[5].replace('-','.')}  /  ${industry || '全部行业'}`;
    ...
    const last = totals[5], previous = totals[4];
    const difference = previous ? `${last>=previous?'+':''}${((last-previous)/previous*100).toFixed(1)}%` : '—';
    $('#trend-headline').innerHTML = `<b>${last}</b><span>本月新增</span><em>环比 ${difference}</em>`;
  ...
  monthControl.onchange=render;
  $('#cockpit-industry').onchange=render;
  $('#cockpit-reset').onclick=()=>{monthControl.value=baseMonth;$('#cockpit-industry').value='';hiddenIndustries.clear();render();};
  render();
}
