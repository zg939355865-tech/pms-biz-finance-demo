  ...
  for (const testCase of activeCases) {
    for (const width of [1440, 2048]) {
      ...
      if (testCase.code === 'project') {
        ...
        const projectSelect = search.locator('select[data-field="projectCode"]');
        expect(await projectSelect.count() === 1, `${testCase.code}@${width}: 项目查询条件不是下拉框`);
        expect(await projectSelect.locator('option').nth(1).innerText() === 'PRJ2026001 华中智能工厂一期', `${testCase.code}@${width}: 项目下拉未展示编码和名称`);
        ...
        expect((await page.locator('#project-cost-trend-chart .section-title').innerText()).trim() === '会计月成本趋势', `${testCase.code}@${width}: 趋势图标题错误`);
        ...
        expect(JSON.stringify(tableHeaders) === JSON.stringify(['年度-会计期间', '项目编码', '项目名称', '项目类型', '项目状态', '项目经理', '成本总计', '服务采购', '物资消耗', '费用报销', '人工成本', '折旧摊销', '间接费用']), `${testCase.code}@${width}: 项目成本汇总字段或顺序错误`);
      ...
      if (testCase.code === 'department') {
        ...
        expect((await page.locator('#department-cost-trend-chart .section-title').innerText()).trim() === '会计月成本趋势', `${testCase.code}@${width}: 趋势图标题错误`);
        const trendAndSearchWidths = await page.evaluate(() => ({
      ...
      if (width === 1440) {
        if (testCase.code === 'project') {
          ...
          await search.locator('select[data-field="projectCode"]').selectOption('PRJ2026002');
          await search.locator('[data-act="query"]').click();
