  ...
  while ((start = lines.findIndex((l, i) => i > start && new RegExp('^\\s*"' + arrayKey + '": \\[$').test(l))) >= 0) {
    ...
    while (i < end) {
      if (lines[i] === objectIndent + "{") {
        let j = i + 1;
        while (j < end && lines[j] !== objectIndent + "}") j += 1;
        const text = lines.slice(i, j + 1).join("\n").replace(/,(\s*[}\]])/g, "$1");
        let row = null;
        try { row = JSON.parse(text); } catch (error) { row = null; }
        if (row && Object.prototype.hasOwnProperty.call(row, "taxIncludedUnitPrice") && !Object.prototype.hasOwnProperty.call(row, "estimatedUnitPrice")) {
          ...
          report.push(arrayKey + " " + key + " " + row.itemName + " 暂估=" + value + " 含税=" + row.taxIncludedUnitPrice);
        }
        i = j + 1;
      } else i += 1;
    }
...
processArray("orderItems", false);

fs.writeFileSync(p, lines.join(eol), "utf8");
const parsed = JSON.parse(fs.readFileSync(p, "utf8"));
console.log(report.join("\n"));
console.log("--- verify ---");
console.log("availableOrderLines:", parsed.mockData.availableOrderLines.map((r) => r.purchaseOrderNo + "/" + r.purchaseOrderLineNo + " 暂估" + r.estimatedUnitPrice + " 含税" + r.taxIncludedUnitPrice).join(" ; "));
console.log("orderItems:", JSON.stringify(parsed.mockData.orderItems.map((r) => [r.estimatedUnitPrice, r.taxIncludedUnitPrice])));
console.log("invoices orderItems:", JSON.stringify(parsed.mockData.invoices.map((v) => (v.orderItems || []).map((r) => [r.estimatedUnitPrice, r.taxIncludedUnitPrice]))));
