  ...
  while ((start = lines.findIndex((l, i) => i > start && new RegExp('^\\s*"' + arrayKey + '": \\[$').test(l))) >= 0) {
    ...
    while (i < end) {
      if (lines[i] === objectIndent + "{") {
        ...
        let row = null;
        try { row = JSON.parse(lines.slice(i, j + 1).join("\n").replace(/,\s*}$/,"}")); } catch (error) { row = null; }
...
console.log(parsed.mockData.availableOrderLines.map((r) => r.purchaseOrderNo + "/" + r.purchaseOrderLineNo + " " + r.itemName + " 暂估" + r.estimatedUnitPrice + " 含税" + r.taxIncludedUnitPrice).join("\n"));
